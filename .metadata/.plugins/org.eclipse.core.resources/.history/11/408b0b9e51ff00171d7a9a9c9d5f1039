...
public class AccountDAOImpl implements AccountDAO{
	...
	public ResultMap insertAccount(Command cmd) {
		...
		try {
			...
			.getConnection()
			.createStatement()
			.executeUpdate(String.format(sql.toString(), cmd.getData(), 0 ,InitCommand.cmd.getData(),"custom_num.nextval" ));	
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ResultMap selectAccountById(Command cmd) {
		...
		try {
			...
			ResultSet rs = DatabaseFactory
							...
							.getConnection()
							.createStatement()
							.executeQuery(String.format(sql.toString(),cmd.getData()));
			while(rs.next()) {
				mem = new MemberBean();
	...
	@Override
	public ResultMap selectKakaoById(Command cmd) {
