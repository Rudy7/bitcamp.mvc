			...
			<article>
				<table id="memberInfo">
					...
					<tr>
						...
						<td>NO</td>
						<td>${sessionScope.user.phone}</td>      <%--  ${sessionScope.user.phone} --%>
					</tr>
					<tr>
						<td>이름</td>
						<td>${sessionScope.user.name}</td>
						<td>이메일</td>
						<td>${sessionScope.user.email}</td>          <%-- ${sessionScope.user.email} --%>
					</tr>
					<tr>
						<td>성별</td>
						<td>남자</td>
						<td>주소</td>
						<td>${sessionScope.user.addr}</td>          <%-- ${sessionScope.user.addr} --%>
					</tr>
					<tr>
						<td>ACCOUNT</td>
						<td colspan="3">${sessionScope.account.accountNum}</td>  <!-- user에는 어카운트넘버가 없다 -->
						<!-- <td> <td/>
						<td> <td/> -->
					</tr>
				</table> <br />
